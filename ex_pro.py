...
with open('mio_file_output.txt', 'r') as f:
	...
	for raw_line in data:
		...
		nums_ls = [x.replace('"', '') for x in split_line] # get rid of the quotation marks and convert to integer
...
legend_elements = [mpatches.Patch(color='#4B0082', label='< 1.0'),
   mpatches.Patch(color='#000080', label='1.0 - 2.0'),
   mpatches.Patch(color='#00BFFF', label='2.0 - 3.0'),
   mpatches.Patch(color='#AFEEEE', label='3.0 - 4.0'),
   mpatches.Patch(color='#00FFFF', label='4.0 - 5.0'),
   mpatches.Patch(color='#66CDAA', label='5.0 - 6.0'),
   mpatches.Patch(color='#90EE90', label='6.0 - 7.0'),
   mpatches.Patch(color='#ADFF2F', label='7.0 - 8.0'),
   mpatches.Patch(color='#FFFF00', label='8.0 - 9.0'),
   mpatches.Patch(color='#B8860B', label='9.0 - 10.0'),
   mpatches.Patch(color='#FA8072', label='10.0 - 11.0'),
   mpatches.Patch(color='#FF0000', label='11.0 - 12.0'),
   mpatches.Patch(color='#800000', label='> 12.0')]
...
plt.axvline(mean_1, color='blue', linestyle='dashed', linewidth=1, label=f'Media Pop t < 1.0 Gyr: {mean_1:.2f}')
plt.axvline(median_1, color='black', linestyle='dashed', linewidth=1, label=f'Mediana t < 1.0 Gyr: {median_1:.2f}')
plt.axvline(mean_2, color='red', linestyle='dashed', linewidth=1, label=f'Media Pop 1.0 Gyr < t < 5.0 Gyr: {mean_2:.2f}')
plt.axvline(median_2, color='yellow', linestyle='dashed', linewidth=1, label=f'Mediana Pop 2.0 Gyr < t < 5.0 Gyr: {median_2:.2f}')
plt.axvline(mean_3, color='orange', linestyle='dashed', linewidth=1, label=f'Media Pop 5.0 Gyr < t < 13.0 Gyr: {mean_3:.2f}')
plt.axvline(median_3, color='pink', linestyle='dashed', linewidth=1, label=f'Mediana Pop 5.0 Gyr < t < 13.0 Gyr: {median_3:.2f}')
plt.legend()
...
legend_elements = [mpatches.Patch(color='red', label='< 1.0'),
   mpatches.Patch(color='blue', label='1.0 - 5.0'),
   mpatches.Patch(color='green', label='5.0 - 13.0')]
